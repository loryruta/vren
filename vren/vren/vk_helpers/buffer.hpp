...
namespace vren::vk_utils
{
    ...
    struct buffer
    {
        ...
        template <typename _t = void> inline _t* get_mapped_pointer()
        {
    ...
    vren::vk_utils::buffer alloc_host_visible_buffer(
        vren::context const& ctx, VkBufferUsageFlags buffer_usage, size_t size, bool persistently_mapped = false
    );

    // TODO Rename to alloc_device_only_buffer
    vren::vk_utils::buffer
    alloc_device_only_buffer(vren::context const& ctx, VkBufferUsageFlags buffer_usage, size_t size);
    ...
    vren::vk_utils::buffer
    create_device_only_buffer(vren::context const& ctx, VkBufferUsageFlags buffer_usage, void const* data, size_t size);
