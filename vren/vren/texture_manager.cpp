...
VkDescriptorSet vren::texture_manager_descriptor_pool::allocate_descriptor_set(VkDescriptorPool descriptor_pool, VkDescriptorSetLayout descriptor_set_layout)
{
...
vren::texture_manager_descriptor_pool::texture_manager_descriptor_pool(vren::context const& context, uint32_t max_sets, std::span<VkDescriptorPoolSize> const& pool_sizes) :
	...
	VkDescriptorSetLayoutBinding bindings[]{
		{
			...
			.descriptorCount = 65536, // TODO (VERY IMPORTANT) k_max_variable_count_descriptor_count used both here and by the shader parser
...
{
	m_descriptor_set = std::make_unique<vren::pooled_vk_descriptor_set>(
		m_descriptor_pool.acquire(m_descriptor_set_layout.m_handle)
	);
	vren::vk_utils::set_object_name(*m_context, VK_OBJECT_TYPE_DESCRIPTOR_SET, (uint64_t) m_descriptor_set->m_handle.m_descriptor_set, "texture_manager");
