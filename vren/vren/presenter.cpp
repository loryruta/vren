...
vren::swapchain::swapchain(
    ...
    m_frame_data(vren::create_array<vren::swapchain_frame_data, VREN_MAX_FRAME_IN_FLIGHT_COUNT>(
        [&](uint32_t idx)
        {
    ...
    ))
{
    ...
    for (VkImage image : m_images)
    {
        ...
        vren::vk_utils::set_object_name(
            *m_context, VK_OBJECT_TYPE_IMAGE_VIEW, (uint64_t) image_view.m_handle, "swapchain_image_view"
        );
    ...
    m_swapchain_recreate_callback(swapchain_recreate_callback)
{
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(
        context.m_physical_device, m_present_queue_family_idx, m_surface->m_handle, &has_support
    );
    if (!has_support)
    {
...
VkSurfaceFormatKHR vren::presenter::pick_surface_format(vren::vk_utils::surface_details const& surf_details)
{
    for (VkSurfaceFormatKHR surf_format : surf_details.m_surface_formats)
    {
        if (surf_format.format ==
            VK_FORMAT_B8G8R8A8_UNORM /* && surf_format.colorSpace == VK_COLORSPACE_SRGB_NONLINEAR_KHR*/)
        {
            return surf_format;
        }
    }
...
{
    // If a swapchain was already created, waits for all the frames attached to the old swapchain to end before
    // destroying it.
    if (m_swapchain)
    {
        ...
        {
            VREN_CHECK(
                vkWaitForFences(m_context->m_device, 1, &frame.m_frame_fence.m_handle, VK_TRUE, UINT64_MAX), m_context
            );
        }
    }
    ...
    VREN_INFO(
        "[presenter] Swapchain re-created: Size {}x{} - Surface format: {:#x} - Surface color-space: {:#x} - Present "
        "mode: "
        "{:#x} - Image count: {} (min. requested: {})\n",
        width,
        height,
        surface_format.format,
        surface_format.colorSpace,
        present_mode,
        m_swapchain->m_images.size(),
        min_image_count
    );
...
void vren::presenter::present(render_func_t const& render_func)
{
    ...
    VREN_CHECK(
        vkWaitForFences(m_context->m_device, 1, &frame_data.m_frame_fence.m_handle, VK_TRUE, UINT64_MAX), m_context
    );
    ...
    /* Command buffer init */
    auto cmd_buf =
        std::make_shared<vren::pooled_vk_command_buffer>(m_context->m_toolbox->m_graphics_command_pool.acquire());
    ...
    result = vkAcquireNextImageKHR(
        ...
        UINT64_MAX,
        frame_data.m_image_available_semaphore.m_handle,
        VK_NULL_HANDLE,
        &img_idx
    );
    ...
    VREN_CHECK(
        vkQueueSubmit(m_context->m_graphics_queue, 1, &submit_info, frame_data.m_frame_fence.m_handle), m_context
    );
    ...
    m_current_frame_idx =
        (m_current_frame_idx + 1) % std::min<uint32_t>(VREN_MAX_FRAME_IN_FLIGHT_COUNT, m_swapchain->m_images.size());
}

// --------------------------------------------------------------------------------------------------------------------------------

vren::render_graph_t vren::blit_color_buffer_to_swapchain_image(
    ...
    node->set_callback(
        ...
        )
        {
            VkImageBlit image_blit{
                ...
                .dstOffsets = {{0, 0, 0}, {(int32_t) swapchain_image_width, (int32_t) swapchain_image_height, 1}},
            };
            vkCmdBlitImage(
                command_buffer,
...
vren::render_graph_t
vren::transit_swapchain_image_to_present_layout(vren::render_graph_allocator& allocator, VkImage swapchain_image)
{
    auto node = allocator.allocate();
