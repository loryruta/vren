...
vren::basic_model_draw_buffer vren::basic_model_uploader::upload(vren::context const& context, vren::model const& model)
{
    ...
    vren::basic_model_draw_buffer draw_buffer{
        .m_name = model.m_name,
        .m_vertex_buffer = vren::vk_utils::create_device_only_buffer(
            context,
            VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
            model.m_vertices.data(),
            model.m_vertices.size() * sizeof(vren::vertex)
        ),
        .m_index_buffer = vren::vk_utils::create_device_only_buffer(
            context, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, model.m_indices.data(), model.m_indices.size() * sizeof(uint32_t)
        ),
        .m_instance_buffer = vren::vk_utils::create_device_only_buffer(
            context,
            VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
            model.m_instances.data(),
            model.m_instances.size() * sizeof(vren::mesh_instance)
        ),
