...
#ifdef VREN_USE_MESH_OPTIMIZER
        max_meshlets_per_mesh = meshopt_buildMeshletsBound(
            mesh.m_index_count, vren::k_max_meshlet_vertex_count, vren::k_max_meshlet_primitive_count
        );
#else
...
void vren::model_clusterizer::clusterize_mesh(
    vren::clusterized_model& output, vren::model const& model, vren::model::mesh const& mesh
)
{
#ifdef VREN_USE_MESH_OPTIMIZER
    size_t meshopt_max_meshlets = meshopt_buildMeshletsBound(
        mesh.m_index_count, vren::k_max_meshlet_vertex_count, vren::k_max_meshlet_primitive_count
    );
    ...
    output.m_meshlet_triangles.resize(
        meshlet_triangles_offset + meshopt_max_meshlets * vren::k_max_meshlet_primitive_count * 3
    );
    ...
    output.m_meshlet_triangles.resize(
        meshlet_triangles_offset + last_meshlet.triangle_offset + ((last_meshlet.triangle_count * 3 + 3) & ~3)
    );
    ...
    for (meshopt_Meshlet const& meshopt_meshlet : meshopt_meshlets)
    {
        ...
        output.m_meshlets.push_back(vren::meshlet{
            ...
            .m_bounding_sphere = {
                .m_center = glm::vec3(
                    meshopt_meshlet_bounds.center[0], meshopt_meshlet_bounds.center[1], meshopt_meshlet_bounds.center[2]
                ),
