...
bool is_triangle_adjacent_to_meshlet(
    uint32_t const* indices, uint32_t triangle, uint32_t const* meshlet_vertices, vren::meshlet const& meshlet
)
{
    uint32_t ai = indices[triangle * 3 + 0], bi = indices[triangle * 3 + 1], ci = indices[triangle * 3 + 2];

    for (uint32_t i = 0; i < meshlet.m_vertex_count; i++)
    {
        if (meshlet_vertices[meshlet.m_vertex_offset + i] == ai ||
            meshlet_vertices[meshlet.m_vertex_offset + i] == bi || meshlet_vertices[meshlet.m_vertex_offset + i] == ci)
        {
            return true;
        }
    }
...
vren::bounding_sphere build_triangle_bounding_sphere(
    float const* vertices, size_t vertex_stride, uint32_t const* indices, uint32_t triangle_idx
)
{
    glm::vec3 p0, p1, p2;
    ...
    return {
        .m_center = midpoint,
        .m_radius =
            glm::max(glm::length(p0 - midpoint), glm::max(glm::length(p1 - midpoint), glm::length(p2 - midpoint)))};
}

bool does_meshlet_contain_triangle(
    float const* vertices, size_t vertex_stride, uint32_t const* indices, uint32_t triangle_idx, vren::meshlet& meshlet
)
{
    glm::vec3 p0, p1, p2;
    get_triangle_vertices(vertices, vertex_stride, indices, triangle_idx, p0, p1, p2);

    return meshlet.m_bounding_sphere.contains(p0
           ) && // Check whether the triangle is fully contained within the meshlet bounding sphere
...
)
{
    ...
    uint32_t a = indices[triangle_idx * 3 + 0], b = indices[triangle_idx * 3 + 1], c = indices[triangle_idx * 3 + 2];
    ...
    if (meshlet.m_vertex_count + (ai == UINT8_MAX) + (bi == UINT8_MAX) + (ci == UINT8_MAX) >=
...
)
{
    ...
    for (uint32_t triangle = 0; triangle < triangle_count; triangle++)
    {
        ...
        if (triangle_picked.at(triangle))
        { // We're searching for the first non-picked triangle
            continue;
        }

        vren::meshlet& meshlet = meshlets[meshlet_count];
        meshlet.m_vertex_offset =
            meshlet_count > 0 ? meshlets[meshlet_count - 1].m_vertex_offset + meshlets[meshlet_count - 1].m_vertex_count
                              : 0;
        meshlet.m_vertex_count = 0;
        meshlet.m_triangle_offset = meshlet_count > 0 ? meshlets[meshlet_count - 1].m_triangle_offset +
                                                            meshlets[meshlet_count - 1].m_triangle_count * 3
                                                      : 0;
        ...
        for (; nearby_triangle_count < std::size(nearby_triangle_queue); nearby_triangle_count++)
        {
            ...
            auto can_pick = [&](uint32_t triangle) -> bool
            {
                return triangle_picked[triangle] == 0;
            };
            kd_tree_search(
                reinterpret_cast<float const*>(triangle_midpoints.data()),
                sizeof(glm::vec3) / sizeof(float),
        ...
        size_t picked_for_adjacency_count = 0, picked_for_bounding_sphere_count = 0;
        while (true)
        {
            ...
            while (true)
            {
                ...
                for (uint32_t i = 0; i < nearby_triangle_count; i++)
                {
                    ...
                    if (triangle_picked[nearby_triangle] != 2 &&
                        is_triangle_adjacent_to_meshlet(indices, nearby_triangle, meshlet_vertices, meshlet))
                    {
                        if (pick_triangle(
                                vertices,
                                vertex_stride,
            ...
            for (uint32_t i = 0; i < nearby_triangle_count; i++)
            {
                uint32_t nearby_triangle = nearby_triangle_queue[i];
                if (triangle_picked[nearby_triangle] != 2 &&
                    does_meshlet_contain_triangle(vertices, vertex_stride, indices, nearby_triangle, meshlet))
                {
                    if (pick_triangle(
                            vertices,
                            vertex_stride,
            ...
            if (picked_triangle_count == 0)
            { // No adjacent triangle and no triangle is contained in the meshlet bounding sphere, there's no more work
                break;
            }
        }

        VREN_DEBUG(
            "Nearby triangle count: {}, Picked for bounding sphere {}, Picked for adjacency {}\n",
            nearby_triangle_count,
            picked_for_bounding_sphere_count,
            picked_for_adjacency_count
        );
