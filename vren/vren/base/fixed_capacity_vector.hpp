...
#define FCV_REQUIRES(...)                                                                                              \
    template <                                                                                                         \
        int FCV_CONCEPT_PP_CAT(_concept_requires_, __LINE__) = 42,                                                     \
        typename ::std::enable_if<(FCV_CONCEPT_PP_CAT(_concept_requires_, __LINE__) == 43) || (__VA_ARGS__), int>::    \
            type = 0> /**/
...
namespace std
{
    namespace experimental
    {
        ...
        namespace fcv_detail
        {
            ...
            static constexpr bool Constructible = is_constructible_v<T, Args...>;

            template <typename T> static constexpr bool CopyConstructible = is_copy_constructible_v<T>;
            ...
            static constexpr bool Movable =
                is_object_v<T> && Assignable<T&, T> && MoveConstructible<T> && is_swappable_v<T&>;
            ...
            template <typename T>
            static constexpr bool OutputIterator = Iterator_<T, output_iterator_tag>{} || ForwardIterator<T>;

            template <typename T>
            static constexpr bool BidirectionalIterator = Iterator_<T, bidirectional_iterator_tag>{};

            template <typename T>
            static constexpr bool RandomAccessIterator = Iterator_<T, random_access_iterator_tag>{};
            ...
            template <
                typename InputIt,
                typename OutputIt,
                FCV_REQUIRES_(InputIterator<InputIt>&& OutputIterator<OutputIt>)>
            ...
            namespace storage
            {
                ...
                template <typename T> struct zero_sized
                {
                    ...
                    {
                        FCV_EXPECT(
                            new_size == 0 && "tried to change size of empty storage to "
                                             "non-zero value"
                        );
                    }
                    ...
                    {
                        FCV_EXPECT(
                            il.size() == 0 && "tried to construct storage::empty from a "
                ...
                private:
                    ...
                    // non-const elements:
                    using data_t =
                        conditional_t<!Const<T>, array<T, Capacity>, const array<remove_const_t<T>, Capacity>>;
                ...
                private:
                    template <typename U, FCV_REQUIRES_(Convertible<U, T>)>
                    static constexpr array<remove_const_t<T>, Capacity> unsafe_recast_init_list(initializer_list<U>& il
                    ) noexcept
                    {
                        FCV_EXPECT(
                            il.size() <= capacity() && "trying to construct storage from an "
                                                       "initializer_list "
                ...
                public:
                    ...
                    ~non_trivial() noexcept(is_nothrow_destructible_v<T>) { unsafe_destroy_all(); }
        ...
        template <typename T, size_t Capacity>
        struct fixed_capacity_vector : private fcv_detail::storage::_t<T, Capacity>
        {
        private:
        ...
        public:
            ...
            /// range)
            constexpr reference operator[](size_type pos) noexcept { return fcv_detail::index(*this, pos); }

            /// Unchecked access to element at index \p pos (UB if index not in
            /// range)
            constexpr const_reference operator[](size_type pos) const noexcept { return fcv_detail::index(*this, pos); }
            ...
            ///
            constexpr reference front() noexcept { return fcv_detail::index(*this, 0); }
            constexpr const_reference front() const noexcept { return fcv_detail::index(*this, 0); }
            ...
            template <
                typename U,
                FCV_REQUIRES_(fcv_detail::Constructible<T, U>&& fcv_detail::Assignable<reference, U&&>)>
            ...
            constexpr iterator
            insert(const_iterator position, const_reference x) noexcept(noexcept(insert(position, size_type(1), x)))
            {
            ...
            constexpr iterator
            insert(const_iterator position, value_type&& x) noexcept(noexcept(move_insert(position, &x, &x + 1)))
            {
                FCV_EXPECT(!full() && "tried insert on full fixed_capacity_vector!");
                assert_iterator_in_range(position);
            ...
            template <
                class InputIt,
                FCV_REQUIRES_(fcv_detail::InputIterator<InputIt>and
                                  fcv_detail::Constructible<value_type, fcv_detail::iterator_reference_t<InputIt>>)>
            constexpr iterator
            insert(const_iterator position, InputIt first, InputIt last) noexcept(noexcept(emplace_back(*first)))
            {
                assert_iterator_in_range(position);
                assert_valid_iterator_pair(first, last);
            ...
            FCV_REQUIRES(fcv_detail::CopyConstructible<T>)
            constexpr iterator insert(const_iterator position, initializer_list<T> il) noexcept(
            ...
            constexpr void resize(size_type sz, T const& value) noexcept(is_nothrow_copy_constructible_v<T>)
            {
                ...
                {
                    FCV_EXPECT(
                        sz <= capacity() && "fixed_capacity_vector cannot be resized to "
        ...
        private:
            ...
            constexpr void emplace_n(size_type n) noexcept(
                (fcv_detail::MoveConstructible<T> && is_nothrow_move_constructible_v<T>) ||
                (fcv_detail::CopyConstructible<T> && is_nothrow_copy_constructible_v<T>)
            )
            {
                FCV_EXPECT(
                    n <= capacity() && "fixed_capacity_vector cannot be "
        ...
        public:
            ...
            ) noexcept(noexcept(insert(begin(), other.begin(), other.end())))
            {
            ...
            ) noexcept(noexcept(move_insert(begin(), other.begin(), other.end())))
            {
                // nothin to assert: size of other cannot exceed capacity
                // because both vectors have the same type
            ...
            template <class InputIt, FCV_REQUIRES_(fcv_detail::InputIterator<InputIt>)>
            constexpr void assign(InputIt first, InputIt last) noexcept(noexcept(clear()
            ) and noexcept(insert(begin(), first, last)))
            {
                if constexpr (fcv_detail::RandomAccessIterator<InputIt>)
                {
                    FCV_EXPECT(last - first >= 0);
        ...
        template <typename T, size_t Capacity>
        constexpr bool
        operator==(fixed_capacity_vector<T, Capacity> const& a, fixed_capacity_vector<T, Capacity> const& b) noexcept
        {
