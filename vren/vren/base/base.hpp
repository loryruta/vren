...
namespace vren
{
    ...
    template <typename _t, size_t _size> auto create_array(std::function<_t(uint32_t idx)> const& create_entry_func)
    {
        return detail::create_array(create_entry_func, std::make_index_sequence<_size>());
    }

    template <typename _t, size_t _size> auto create_array(_t value)
    {
        return create_array<_t, _size>(
            [&](uint32_t idx)
            {
                return value;
            }
    ...
    template <typename _integer_t> _integer_t round_to_next_multiple_of(_integer_t value, _integer_t multiple)
    {
        _integer_t reminder = value % multiple;
        return reminder == 0 ? value : (value + multiple - reminder);
    }

    template <typename _integer_t> bool is_power_of(_integer_t n, _integer_t base)
    {
        double exp = glm::log((double) n) / glm::log((double) base);
        return (exp - glm::floor(exp)) < std::numeric_limits<double>::epsilon();
    }

    template <typename _integer_t> _integer_t round_to_next_power_of(_integer_t n, _integer_t base)
    {
        double exp = glm::log((double) n) / glm::log((double) base);
    ...
    template <typename _t, size_t _size> using static_vector_t = std::experimental::fixed_capacity_vector<_t, _size>;
