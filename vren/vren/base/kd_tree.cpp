...
)
{
    ...
    size_t next_node_offset =
        kd_tree_build(points, point_stride, indices, middle, kd_tree, node_offset + 1, max_leaf_point_count);
    ...
    return kd_tree_build(
        points, point_stride, indices + middle, count - middle, kd_tree, next_node_offset, max_leaf_point_count
    );
}
...
)
{
    ...
    if (node.m_axis == 0x3) // Is children
    {
        for (uint32_t i = 0; i < node.m_leaf_count; i++)
        {
            ...
            float distance_squared = (sample[0] - point[0]) * (sample[0] - point[0]) +
