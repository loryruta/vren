...
vren::bucket_sort::bucket_sort(vren::context const& context) :
    ...
    m_count_pipeline(
        [&]()
        {
            vren::shader_module shader_module =
                vren::load_shader_module_from_file(context, ".vren/resources/shaders/bucket_sort_count.comp.spv");
    ...
    m_write_pipeline(
        ...
        {
            vren::shader_module shader_module =
                vren::load_shader_module_from_file(context, ".vren/resources/shaders/bucket_sort_write.comp.spv");
            vren::specialized_shader shader = vren::specialized_shader(shader_module);
            return vren::create_compute_pipeline(context, shader);
        }()
    )
...
{
    return vren::round_to_next_multiple_of(
               length * sizeof(glm::uvec2), (size_t) VREN_MIN_STORAGE_BUFFER_OFFSET_ALIGNMENT
           ) +
...
)
{
    ...
    assert(
        (output_buffer.m_allocation_info.size - output_buffer_offset) >=
        get_required_output_buffer_size(input_buffer_length)
    );

    size_t bucket_count_buffer_offset = output_buffer_offset + vren::round_to_next_multiple_of(
                                                                   input_buffer_length * sizeof(glm::uvec2),
                                                                   (size_t) VREN_MIN_STORAGE_BUFFER_OFFSET_ALIGNMENT
                                                               );
    ...
    // Clear
    vkCmdFillBuffer(
        command_buffer, output_buffer.m_buffer.m_handle, bucket_count_buffer_offset, k_key_size * sizeof(uint32_t), 0
    );
    ...
    m_context->m_toolbox->m_blelloch_scan(
        command_buffer, resource_container, output_buffer, k_key_size, bucket_count_buffer_offset, 1
    );
