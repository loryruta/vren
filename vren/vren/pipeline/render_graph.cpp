...
)
{
    ...
    while (stepping_graph[_1].size() > 0)
    {
        ...
        for (vren::render_graph_node_index_t node_idx : stepping_graph[_1])
        {
            ...
            {
                auto const& next_nodes = forward_traversal ? allocator.get_node_at(node_idx)->get_next_nodes()
                                                           : allocator.get_node_at(node_idx)->get_previous_nodes(
                                                             ); // Maybe constexpr condition would make it go faster?
...
vren::render_graph_t
vren::render_graph_get_start(vren::render_graph_allocator& allocator, vren::render_graph_t const& graph)
{
    return graph;
}

vren::render_graph_t
vren::render_graph_get_end(vren::render_graph_allocator& allocator, vren::render_graph_t const& graph)
{
    vren::render_graph_t result;

    vren::render_graph_traverse(
        allocator,
        graph,
        [&](vren::render_graph_node_index_t node_idx)
        {
...
vren::render_graph_t vren::render_graph_concat(
    vren::render_graph_allocator& allocator, vren::render_graph_t const& left, vren::render_graph_t const& right
)
{
    if (left.empty())
    {
...
void vren::detail::render_graph_executor::execute(
    vren::render_graph_allocator& allocator, vren::render_graph_t const& graph
)
{
#ifdef VREN_LOG_RENDER_GRAPH_DETAILED
    VREN_DEBUG(
        ...
        for (vren::render_graph_node_index_t node_idx : stepping_graph[_1])
        {
            ...
            for (auto const& image_access : node->get_image_accesses())
            {
                vren::render_graph_traverse(
                    allocator,
                    {node_idx},
                    [&](vren::render_graph_node_index_t node_idx_2) -> bool
                    {
            ...
            for (auto const& buffer_access : node->get_buffer_accesses())
            {
                vren::render_graph_traverse(
                    ...
                    [&](vren::render_graph_node_index_t node_idx_2) -> bool
                    {
                        ...
                        for (vren::render_graph_node_buffer_access const& buffer_access_2 :
                             node_2->get_buffer_accesses())
                        {
                            if (buffer_access.m_buffer_idx == buffer_access_2.m_buffer_idx)
                            {
#ifdef VREN_LOG_RENDER_GRAPH_DETAILED
                                vren::render_graph_buffer_info const& buffer_info =
                                    allocator.get_buffer_info_at(buffer_access.m_buffer_idx);
                                VREN_DEBUG0(
                                    "[render_graph] Buffer barrier for: {}, node 1: {}, node 2: {}\n",
...
void vren::render_graph_executor::make_initial_image_layout_transition(
    vren::render_graph_node const& node, vren::render_graph_node_image_access const& image_access
)
{
...
#ifdef VREN_LOG_RENDER_GRAPH_DETAILED
    VREN_DEBUG(
        "[render_graph] Initial layout transition for {} to {}\n",
...
void vren::render_graph_executor::execute_node(vren::render_graph_node const& node)
{
    ...
    VREN_DEBUG(
        "[render_graph] {}\n",
        fmt::format(
            fmt::fg(fmt::color::lime), "Executing node: {}", fmt::format(fmt::fg(fmt::color::yellow), node.get_name())
        )
    );
#endif
...
{
    vren::render_graph_image_info const& image_1 =
        node_1.get_allocator()->get_image_info_at(image_access_1.m_image_idx);
    // vren::render_graph_image_info const& image_2 =
    // node_2.get_allocator()->get_image_info_at(image_access_2.m_image_idx);

#ifdef VREN_LOG_RENDER_GRAPH_DETAILED
    VREN_DEBUG(
        "[render_graph] Image barrier for {}, mip: {}, layer: {}\n",
        fmt::format(fmt::fg(fmt::color::fuchsia), image_1.m_name),
        image_1.m_mip_level,
        image_1.m_layer
    );
    VREN_DEBUG(
        "[render_graph] - Src node: {}, stage: {} ({}), access: {}\n",
    ...
    VREN_DEBUG(
        "[render_graph] - Dst node: {}, stage: {} ({}), access: {}\n",
        fmt::format(fmt::fg(fmt::color::yellow), node_2.get_name()),
    ...
    vkCmdPipelineBarrier(
        m_command_buffer,
        node_1.get_dst_stage(),
        node_2.get_src_stage(),
        NULL,
        0,
        nullptr,
        0,
...
{
    vren::render_graph_buffer_info const& buffer_1 =
        node_1.get_allocator()->get_buffer_info_at(buffer_access_1.m_buffer_idx);
    vren::render_graph_buffer_info const& buffer_2 =
        node_2.get_allocator()->get_buffer_info_at(buffer_access_2.m_buffer_idx);
...
void vren::render_graph_dumper::make_initial_image_layout_transition(
    vren::render_graph_node const& node, vren::render_graph_node_image_access const& image_access
)
{
    vren::render_graph_image_info const& image_info = node.get_allocator()->get_image_info_at(image_access.m_image_idx);
    *m_output << fmt::format(
        "node_{}_image_{} [color=red, fillcolor=orange, label=\"{} ({})\"];\n",
        node.get_idx(),
        image_access.m_image_idx,
        image_info.m_name,
        m_execution_order
    );
    m_execution_order++;
...
{
    *m_output << fmt::format(
        "node_{}_image_{} -> node_{}_image_{} [color=red, xlabel=\"({})\"]",
        node_1.get_idx(),
        image_access_1.m_image_idx,
        node_2.get_idx(),
        image_access_2.m_image_idx,
        m_execution_order
    );
    m_execution_order++;
}
...
{
    *m_output << fmt::format(
        "node_{}_buffer_{} -> node_{}_buffer_{} [color=red, xlabel=\"({})\"]",
        node_1.get_idx(),
        buffer_access_1.m_buffer_idx,
        node_2.get_idx(),
        buffer_access_2.m_buffer_idx,
        m_execution_order
    );
    m_execution_order++;
}
    ...
    vren::render_graph_traverse(
        ...
        [&](vren::render_graph_node_index_t node_idx)
        {
            ...
            *m_output << fmt::format("node_{} [shape=rect, label=\"{}\"];\n", node_idx, node->get_name());
            ...
            for (vren::render_graph_node_image_access const& image_access : node->get_image_accesses())
            {
                vren::render_graph_image_info const& image_info =
                    node->get_allocator()->get_image_info_at(image_access.m_image_idx);

                *m_output << fmt::format(
                    "node_{}_image_{} [shape=rect, label=\"{}\", style=filled, fillcolor=yellow, color=gray, "
                    "height=0];\n",
                    node_idx,
                    image_access.m_image_idx,
                    image_info.m_name
                ...
                *m_output << fmt::format(
                    "node_{} -> node_{}_image_{} [arrowhead=none, color=gray];\n",
                    node_idx,
                    node_idx,
                    image_access.m_image_idx
                );
            ...
            {
                vren::render_graph_buffer_info const& buffer_info =
                    node->get_allocator()->get_buffer_info_at(buffer_access.m_buffer_idx);

                *m_output << fmt::format(
                    "node_{}_buffer_{} [shape=rect, label=\"{}\", style=filled, fillcolor=yellow, color=gray, "
                    "height=0];\n",
                    node_idx,
                    buffer_access.m_buffer_idx,
                    buffer_info.m_name
                );
                *m_output << fmt::format(
                    "node_{} -> node_{}_buffer_{} [arrowhead=none, color=gray];\n",
                    node_idx,
                    node_idx,
                    buffer_access.m_buffer_idx
                );
...
vren::render_graph_t vren::clear_color_buffer(
    vren::render_graph_allocator& allocator, VkImage color_buffer, VkClearColorValue clear_color_value
)
{
...
vren::render_graph_t vren::clear_depth_stencil_buffer(
    vren::render_graph_allocator& allocator, VkImage depth_buffer, VkClearDepthStencilValue clear_depth_stencil_value
)
{
    auto node = allocator.allocate();
