...
namespace vren
{
    ...
    public:
        inline uint32_t get_image_width(uint32_t level) const { return glm::max(m_base_width >> level, 1u); }

        inline uint32_t get_image_height(uint32_t level) const { return glm::max(m_base_height >> level, 1u); }

        inline uint32_t get_level_count() const { return m_level_count; }

        inline VkImage get_image() const { return m_image.m_image.m_handle; }

        inline VkImageView get_image_view() const { return m_image_view.m_handle; }

        inline VkImageView get_level_image_view(uint32_t level) const { return m_level_image_views.at(level).m_handle; }

        inline VkSampler get_sampler() const { return m_sampler.m_handle; }
    ...
    private:
        ...
        vren::vk_sampler m_depth_buffer_sampler; // Used to copy the depth buffer in a depth image format (D32 sfloat)
                                                 // to a color image format (R32 sfloat)
