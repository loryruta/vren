...
vren::construct_light_array_bvh::construct_light_array_bvh(vren::context const& context) :
    ...
    {
        vren::shader_module shader_module = vren::load_shader_module_from_file(context, ".vren/resources/shaders/discretize_light_positions.comp.spv");
    ...
    {
        vren::shader_module shader_module = vren::load_shader_module_from_file(context, ".vren/resources/shaders/init_light_array_bvh.comp.spv");
        vren::specialized_shader shader = vren::specialized_shader(shader_module);
        return vren::create_compute_pipeline(context, shader);
    }())
{
...
size_t vren::construct_light_array_bvh::get_required_bvh_buffer_size(uint32_t light_count)
{
    return glm::max<size_t>(
        glm::max<size_t>(
            vren::build_bvh::get_buffer_size(light_count),
            light_count * sizeof(glm::uvec2) // Morton codes
        ),
        (light_count + 2) * sizeof(glm::vec3) // Reduction output + min/max
    );
}

size_t vren::construct_light_array_bvh::get_required_light_index_buffer_size(uint32_t light_count)
{
    return glm::max<size_t>(
        light_count * sizeof(glm::uvec2),
        (light_count + 2) * sizeof(glm::vec3)
    );
}
...
{
    size_t point_light_count = light_array.m_point_light_count;
    size_t light_count = light_array.m_point_light_count; // TODO: at the moment only point lights, but also spot lights could be supported
    ...
    {
        size_t bvh_size = vren::build_bvh::get_buffer_size(light_count);
        ...
        // Reduce the light position buffer to scratch_buffer_1 in order to find max, the max value is written 1 slot away from light positions' end (at `(light_count + 1) * sizeof(glm::vec4)`)

        m_context->m_toolbox->m_reduce_vec4_max(command_buffer, resource_container, light_position_buffer, light_count, 0, scratch_buffer_1, 2 * sizeof(glm::vec4), 1);

        // Find min, the min value is written soon after light positions' end (at `light_count * sizeof(glm::vec4)`)
        ...
        buffer_memory_barrier = {
            ...
            .size = light_count * sizeof(glm::vec4)
        };
        ...
        m_context->m_toolbox->m_reduce_vec4_min(command_buffer, resource_container, light_position_buffer, light_count, 0, scratch_buffer_1, sizeof(glm::vec4), 1);

        // Now we have (min, max) next to eachother at `light_count * sizeof(glm::vec4)`,
        // we use the initial scratch_buffer_1 memory to store morton codes: the discretization of light positions
        ...
        buffer_memory_barrier = {
            ...
            .buffer = scratch_buffer_1.m_buffer.m_handle,
            .offset = 0,
            .size = light_count * sizeof(glm::vec3)
        };
        vkCmdPipelineBarrier(command_buffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, NULL, 0, nullptr, 1, &buffer_memory_barrier, 0, nullptr);
        ...
        vren::vk_utils::write_buffer_descriptor(*m_context, descriptor_set->m_handle.m_descriptor_set, 0, scratch_buffer_1.m_buffer.m_handle, 2 * sizeof(glm::vec4), light_count * sizeof(glm::vec4));
        ...
        m_discretize_light_positions_pipeline.dispatch(command_buffer, light_count, 1, 1);

        // Sort morton codes using bucket_sort primitive to improve lights locality before BVH construction,
        // the sorted results are stored in scratch_buffer_2
        ...
        m_context->m_toolbox->m_bucket_sort(command_buffer, resource_container, scratch_buffer_1, light_count, 0, scratch_buffer_2, 0);

        // Copy sorted morton codes to BVH leaves on scratch_buffer_1, after that we delegate BVH construction to the build_bvh primitive
        ...
        buffer_memory_barrier = { // Also wait for scratch_buffer_1 to be free as it will be used to store the BVH
            ...
            .buffer = scratch_buffer_1.m_buffer.m_handle,
            .offset = 0,
            .size = glm::max(light_count * sizeof(glm::uvec2), bvh_size)
        };
        vkCmdPipelineBarrier(command_buffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, NULL, 0, nullptr, 1, &buffer_memory_barrier, 0, nullptr);
        ...
        vren::vk_utils::write_buffer_descriptor(*m_context, descriptor_set->m_handle.m_descriptor_set, 0, point_light_buffer.m_buffer.m_handle, point_light_count * sizeof(glm::vec4), light_count * sizeof(glm::vec4));
        vren::vk_utils::write_buffer_descriptor(*m_context, descriptor_set->m_handle.m_descriptor_set, 1, light_position_buffer.m_buffer.m_handle, light_count * sizeof(glm::vec4), 0);
        ...
        vren::vk_utils::write_buffer_descriptor(*m_context, descriptor_set->m_handle.m_descriptor_set, 3, scratch_buffer_1.m_buffer.m_handle, bvh_size, 0);
        ...
        m_context->m_toolbox->m_build_bvh(command_buffer, resource_container, scratch_buffer_1, light_count, nullptr);
    });
