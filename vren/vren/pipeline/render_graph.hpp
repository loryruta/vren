...
namespace vren
{
    ...
    struct render_graph_image_info
    {
        ...
        {
            return m_image == other.m_image && m_image_aspect == other.m_image_aspect &&
                   m_mip_level == other.m_mip_level && m_layer == other.m_layer;
        }
    };
    ...
    struct render_graph_image_info_hash
    {
        ...
        {
            return size_t(
                ((uint64_t) element.m_image) ^ element.m_image_aspect ^
                ((uint64_t(element.m_mip_level) << 32) | element.m_layer)
            );
        };
    };
    ...
    struct render_graph_buffer_info
    {
        ...
        inline bool operator==(vren::render_graph_buffer_info const& other) const { return m_buffer == other.m_buffer; }
    };
    ...
    public:
        ...
        inline vren::render_graph_allocator* get_allocator() const { return m_allocator; }

        inline auto get_idx() const { return m_allocation_idx; }

        inline auto get_name() const { return m_name; }

        inline void set_name(char const* name) { m_name = name; }
        ...
        inline auto const& get_image_accesses() const { return m_image_accesses; }
        ...
        inline auto const& get_previous_nodes() const { return m_previous_nodes; }

        inline auto const& get_next_nodes() const { return m_next_nodes; }
        ...
        // Forward decl
        void add_image(
            vren::render_graph_image_info const& image_info, VkImageLayout image_layout, VkAccessFlags access_flags
        );
    ...
    inline void vren::render_graph_node::add_image(
        vren::render_graph_image_info const& image_info, VkImageLayout image_layout, VkAccessFlags access_flags
    )
    {
    ...
    inline void
    vren::render_graph_node::add_buffer(vren::render_graph_buffer_info const& buffer_info, VkAccessFlags access_flags)
    {
        m_buffer_accesses.push_back(vren::render_graph_node_buffer_access{
            .m_buffer_idx = m_allocator->subscribe_buffer_info(buffer_info), .m_access_flags = access_flags});
    }
    ...
    using render_graph_t =
        vren::static_vector_t<vren::render_graph_node_index_t, vren::render_graph_allocator::k_max_allocable_nodes>;
    ...
    vren::render_graph_t
    render_graph_get_start(vren::render_graph_allocator& allocator, vren::render_graph_t const& graph);
    vren::render_graph_t
    render_graph_get_end(vren::render_graph_allocator& allocator, vren::render_graph_t const& graph);

    template <typename... _nodes>
        requires(std::is_same_v<vren::render_graph_node, typename std::remove_pointer<_nodes>::type> && ...)
    vren::render_graph_t render_graph_gather(_nodes... nodes)
    {
        return {(nodes->get_idx(), ...)};
    }

    vren::render_graph_t render_graph_concat(
    ...
    public:
        ...
        inline vren::render_graph_t const& get_head() const { return m_head; }

        inline vren::render_graph_t const& get_tail() const { return m_tail; }
    ...
    namespace detail
    {
        ...
        {
        protected:
            virtual void make_initial_image_layout_transition(
    ...
    {
    protected:
        void make_initial_image_layout_transition(
            vren::render_graph_node const& node, vren::render_graph_node_image_access const& image_access
        ) override;
