...
)
{
    ...
    depth_buffer_pyramid.add_render_graph_node_resources(
        *node, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_SHADER_READ_BIT
    );
    ...
    node->set_callback(
        [&](uint32_t frame_idx, VkCommandBuffer command_buffer, vren::ResourceContainer& resource_container)
        {
            ...
            vkCmdSetScissor(
                command_buffer, 0, 1, &render_area
            ); // TODO bind and then set viewport (embed mesh_shader_draw_pass here)

            m_mesh_shader_draw_pass.render(
                frame_idx,
                command_buffer,
                resource_container,
                camera_data,
