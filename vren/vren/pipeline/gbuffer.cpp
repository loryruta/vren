    ...
    ))
{
    ...
    vren::vk_utils::set_object_name(
        context, VK_OBJECT_TYPE_SAMPLER, reinterpret_cast<uint64_t>(m_sampler.m_handle), "gbuffer_sampler"
    );
}

void vren::gbuffer::add_render_graph_node_resources(
    vren::render_graph_node& node, VkImageLayout image_layout, VkAccessFlags access_flags
) const
{
    node.add_image(
        {.m_name = "gbuffer_normal",
         .m_image = m_normal_buffer.get_image(),
         .m_image_aspect = VK_IMAGE_ASPECT_COLOR_BIT},
        image_layout,
        access_flags
    );
    node.add_image(
        {.m_name = "gbuffer_texcoord",
         .m_image = m_texcoord_buffer.get_image(),
         .m_image_aspect = VK_IMAGE_ASPECT_COLOR_BIT},
        image_layout,
        access_flags
    );
    node.add_image(
        {.m_name = "gbuffer_material_index",
         .m_image = m_material_index_buffer.get_image(),
         .m_image_aspect = VK_IMAGE_ASPECT_COLOR_BIT},
        image_layout,
        access_flags
    );
...
void vren::gbuffer::write_descriptor_set(
...
) const
{
    vren::vk_utils::write_combined_image_sampler_descriptor(
        *m_context,
        ...
        0,
        m_sampler.m_handle,
        m_normal_buffer.m_image_view.m_handle,
    ...
    vren::vk_utils::write_combined_image_sampler_descriptor(
        *m_context,
        ...
        1,
        m_sampler.m_handle,
        m_texcoord_buffer.m_image_view.m_handle,
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
    );
    vren::vk_utils::write_combined_image_sampler_descriptor(
        *m_context,
        ...
        2,
        m_sampler.m_handle,
        m_material_index_buffer.m_image_view.m_handle,
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
    );
    vren::vk_utils::write_combined_image_sampler_descriptor(
        *m_context,
...
vren::render_graph_t vren::clear_gbuffer(vren::render_graph_allocator& allocator, vren::gbuffer const& gbuffer)
{
    ...
    node->set_callback(
        [&gbuffer](uint32_t frame_idx, VkCommandBuffer command_buffer, vren::ResourceContainer& resource_container)
        {
            ...
            vkCmdClearColorImage(
                command_buffer,
                gbuffer.m_normal_buffer.get_image(),
            ...
            vkCmdClearColorImage(
                command_buffer,
                gbuffer.m_material_index_buffer.get_image(),
                VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                &clear_color,
                1,
                &image_subresource_range
            ...
            vkCmdClearColorImage(
                command_buffer,
                gbuffer.m_texcoord_buffer.get_image(),
                VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                &clear_color,
                1,
                &image_subresource_range
