...
void vren::debug_renderer_draw_buffer::add_lines(vren::debug_renderer_line const* lines, size_t line_count)
{
    ...
    std::for_each(
        ...
        lines,
        lines + line_count,
        [&](vren::debug_renderer_line const& line)
        {
...
void vren::debug_renderer_draw_buffer::add_points(vren::debug_renderer_point const* points, size_t point_count)
{
    ...
    std::for_each(
        ...
        points,
        points + point_count,
        [&](vren::debug_renderer_point const& point)
        {
            uint32_t i = &point - points;

            lines[i * 3 + 0] = {
                .m_from = point.m_position - glm::vec3(k_debug_renderer_point_size, 0, 0),
                .m_to = point.m_position + glm::vec3(k_debug_renderer_point_size, 0, 0),
                .m_color = point.m_color};
            lines[i * 3 + 1] = {
                .m_from = point.m_position - glm::vec3(0, k_debug_renderer_point_size, 0),
                .m_to = point.m_position + glm::vec3(0, k_debug_renderer_point_size, 0),
                .m_color = point.m_color};
            lines[i * 3 + 2] = {
                .m_from = point.m_position - glm::vec3(0, 0, k_debug_renderer_point_size),
                .m_to = point.m_position + glm::vec3(0, 0, k_debug_renderer_point_size),
                .m_color = point.m_color};
        }
    );
...
void vren::debug_renderer_draw_buffer::add_spheres(vren::debug_renderer_sphere const* spheres, size_t sphere_count)
{
    ...
    std::for_each(
        ...
        [&](vren::debug_renderer_sphere const& sphere)
        {
            ...
            for (uint32_t j = 0; j < vren::k_debug_sphere_segment_count; j++)
            {
                ...
                // XZ circle
                p0 = glm::vec3(
                    sphere.m_center.x + sphere.m_radius * glm::cos(a0),
                    sphere.m_center.y,
                    sphere.m_center.z + sphere.m_radius * glm::sin(a0)
                );
                p1 = glm::vec3(
                    sphere.m_center.x + sphere.m_radius * glm::cos(a1),
                    sphere.m_center.y,
                    sphere.m_center.z + sphere.m_radius * glm::sin(a1)
                );

                lines[i * 3 * vren::k_debug_sphere_segment_count + j * 3 + 0] = {
                    .m_from = p0, .m_to = p1, .m_color = sphere.m_color};

                // YZ circle
                p0 = glm::vec3(
                    sphere.m_center.x,
                    sphere.m_center.y + sphere.m_radius * glm::cos(a0),
                    sphere.m_center.z + sphere.m_radius * glm::sin(a0)
                );
                p1 = glm::vec3(
                    sphere.m_center.x,
                    sphere.m_center.y + sphere.m_radius * glm::cos(a1),
                    sphere.m_center.z + sphere.m_radius * glm::sin(a1)
                );

                lines[i * 3 * vren::k_debug_sphere_segment_count + j * 3 + 1] = {
                    .m_from = p0, .m_to = p1, .m_color = sphere.m_color};

                // XY circle
                p0 = glm::vec3(
                    sphere.m_center.x + sphere.m_radius * glm::cos(a0),
                    sphere.m_center.y + sphere.m_radius * glm::sin(a0),
                    sphere.m_center.z
                );
                p1 = glm::vec3(
                    sphere.m_center.x + sphere.m_radius * glm::cos(a1),
                    sphere.m_center.y + sphere.m_radius * glm::sin(a1),
                    sphere.m_center.z
                );

                lines[i * 3 * vren::k_debug_sphere_segment_count + j * 3 + 2] = {
                    .m_from = p0, .m_to = p1, .m_color = sphere.m_color};
            }
        }
    );
...
void vren::debug_renderer_draw_buffer::add_cubes(vren::debug_renderer_cube const* cubes, size_t count)
{
    ...
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        lines.emplace_back<vren::debug_renderer_line>(
            {.m_from = m, .m_to = glm::vec3(M.x, m.y, m.z), .m_color = cube.m_color}
        );
        lines.emplace_back<vren::debug_renderer_line>(
            {.m_from = glm::vec3(M.x, m.y, m.z), .m_to = glm::vec3(M.x, m.y, M.z), .m_color = cube.m_color}
        );
        lines.emplace_back<vren::debug_renderer_line>(
            {.m_from = glm::vec3(M.x, m.y, M.z), .m_to = glm::vec3(m.x, m.y, M.z), .m_color = cube.m_color}
        );
        lines.emplace_back<vren::debug_renderer_line>(
            {.m_from = glm::vec3(m.x, m.y, M.z), .m_to = m, .m_color = cube.m_color}
        );
...
vren::pipeline vren::debug_renderer::create_graphics_pipeline(bool depth_test)
{
    VkVertexInputBindingDescription vtx_bindings[]{
        {
            .binding = 0,
            .stride = sizeof(vren::debug_renderer_vertex),
    ...
    VkVertexInputAttributeDescription vtx_attribs[]{
        {.location = 0,
         ...
         .offset = (uint32_t) offsetof(vren::debug_renderer_vertex, m_position)}, // Position
        {.location = 1,
         .binding = 0,
         .format = VK_FORMAT_R32_UINT,
         .offset = (uint32_t) offsetof(vren::debug_renderer_vertex, m_color)}, // Color
    };
    ...
    //
    vren::shader_module vert_shader_mod =
        vren::load_shader_module_from_file(*m_context, ".vren/resources/shaders/debug_draw.vert.spv");
    vren::shader_module frag_shader_mod =
        vren::load_shader_module_from_file(*m_context, ".vren/resources/shaders/debug_draw.frag.spv");
    ...
    vren::specialized_shader shaders[] = {std::move(vert_shader), std::move(frag_shader)};
         ...
         &draw_buffer](uint32_t frame_idx, VkCommandBuffer command_buffer, vren::resource_container& resource_container)
        {
            ...
            if (world_space)
            {
                pipeline.push_constants(
                    command_buffer, VK_SHADER_STAGE_VERTEX_BIT, &camera_data, sizeof(vren::camera_data)
                );
            }
            else
            {
                vren::camera_data no_camera{
                    .m_view = glm::identity<glm::mat4>(), .m_projection = glm::identity<glm::mat4>()};
                pipeline.push_constants(
                    command_buffer, VK_SHADER_STAGE_VERTEX_BIT, &no_camera, sizeof(vren::camera_data)
                );
            }
            ...
            vkCmdBindVertexBuffers(
                command_buffer, 0, 1, &draw_buffer.m_vertex_buffer.m_buffer->m_buffer.m_handle, offsets
            );
