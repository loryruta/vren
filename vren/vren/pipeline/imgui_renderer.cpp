    ...
    m_render_pass(create_render_pass())
{
    ...
    ImGui_ImplVulkan_LoadFunctions(
        [](char const* func_name, void* user_data)
        {
...
vren::imgui_renderer::~imgui_renderer()
{
    m_descriptor_pool.~vk_descriptor_pool(
    ); // Manually call destructor to ensure these elements are destroyed before the ImGui context
...
)
{
    ...
    node->set_callback(
        [=](uint32_t frame_idx, VkCommandBuffer command_buffer, vren::ResourceContainer& resource_container)
        {
            ...
            // Create a temporary framebuffer (to emulate dynamic rendering feature since ImGui Vulkan backend doesn't
            // support it)
            ...
            auto framebuffer = std::make_shared<vren::vk_framebuffer>(vren::vk_utils::create_framebuffer(
                *m_context, m_render_pass.m_handle, framebuffer_size.width, framebuffer_size.height, attachments
            ));
