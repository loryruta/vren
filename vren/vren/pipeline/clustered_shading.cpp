...
vren::clustered_shading::construct_point_light_bvh::construct_point_light_bvh(vren::context const& context) :
    ...
    m_point_light_position_to_view_space_pipeline(
        [&]()
        {
            vren::shader_module shader_module = vren::load_shader_module_from_file(
                context, ".vren/resources/shaders/clustered_shading/point_light_position_to_view_space.comp.spv"
            );
    ...
    m_discretize_point_light_positions_pipeline(
        ...
        {
            vren::shader_module shader_module = vren::load_shader_module_from_file(
                context, ".vren/resources/shaders/clustered_shading/discretize_point_light_positions.comp.spv"
            );
            vren::specialized_shader shader = vren::specialized_shader(shader_module);
            return vren::create_compute_pipeline(context, shader);
        }()
    ...
    m_init_light_array_bvh_pipeline(
        ...
        {
            vren::shader_module shader_module = vren::load_shader_module_from_file(
                context, ".vren/resources/shaders/clustered_shading/init_light_array_bvh.comp.spv"
            );
            vren::specialized_shader shader = vren::specialized_shader(shader_module);
            return vren::create_compute_pipeline(context, shader);
        }()
...
size_t vren::clustered_shading::construct_point_light_bvh::get_required_bvh_buffer_size(uint32_t point_light_count
) // scratch_buffer_1
{
    ...
    return glm::max<size_t>(
        ...
        VREN_MIN_STORAGE_BUFFER_OFFSET_ALIGNMENT * 3 +
            vren::round_to_next_multiple_of(
                light_count_power_of_2 * sizeof(glm::vec4), VREN_MIN_STORAGE_BUFFER_OFFSET_ALIGNMENT
            ) +
            2 * sizeof(glm::vec4) // Reduction + min/max
    );
}
...
size_t
vren::clustered_shading::construct_point_light_bvh::get_required_point_light_index_buffer_size(uint32_t light_count
) // scratch_buffer_2
{
    return glm::max<size_t>(light_count * sizeof(glm::uvec2), (light_count + 2) * sizeof(glm::vec4));
}
...
)
{
    ...
    assert(
        point_light_index_buffer.m_allocation_info.size >= get_required_point_light_index_buffer_size(point_light_count)
    );
    ...
    // Push constants
    {
        ...
        m_point_light_position_to_view_space_pipeline.push_constants(
            command_buffer, VK_SHADER_STAGE_COMPUTE_BIT, &push_constants, sizeof(push_constants), 0
        );
    }

    // Descriptor set 0
    descriptor_set = std::make_shared<vren::pooled_vk_descriptor_set>(m_context->m_toolbox->m_descriptor_pool.acquire(
        m_point_light_position_to_view_space_pipeline.m_descriptor_set_layouts.at(0)
    ));
    ...
    vren::vk_utils::write_buffer_descriptor(
        *m_context,
        ...
        0,
        light_array.m_point_light_position_buffer.m_buffer.m_handle,
        point_light_count * sizeof(glm::vec4),
        0
    );
    vren::vk_utils::write_buffer_descriptor(
        *m_context,
        descriptor_set->m_handle.m_descriptor_set,
        1,
        view_space_point_light_position_buffer.m_buffer.m_handle,
        point_light_count * sizeof(glm::vec4),
        0
    );

    m_point_light_position_to_view_space_pipeline.bind_descriptor_set(
        command_buffer, 0, descriptor_set->m_handle.m_descriptor_set
    );

    // Dispatch
    m_point_light_position_to_view_space_pipeline.dispatch(
        command_buffer, vren::divide_and_ceil(point_light_count, 1024), 1, 1
    );
    ...
    size_t min_max_offset = VREN_MIN_STORAGE_BUFFER_OFFSET_ALIGNMENT * 3 +
                            vren::round_to_next_multiple_of(
                                point_light_count * sizeof(glm::vec4), VREN_MIN_STORAGE_BUFFER_OFFSET_ALIGNMENT
                            );

    buffer_copies = {
        VkBufferCopy{// min
                     .srcOffset =
                         VREN_MIN_STORAGE_BUFFER_OFFSET_ALIGNMENT + (light_count_power_of_2 - 1) * sizeof(glm::vec4),
                     .dstOffset = min_max_offset,
                     .size = sizeof(glm::vec4)},
        VkBufferCopy{// max
                     .srcOffset = VREN_MIN_STORAGE_BUFFER_OFFSET_ALIGNMENT * 2 +
                                  (light_count_power_of_2 - 1) * sizeof(glm::vec4),
                     .dstOffset = min_max_offset + sizeof(glm::vec4),
                     .size = sizeof(glm::vec4)}};
    vkCmdCopyBuffer(
        command_buffer, scratch_buffer_1.m_buffer.m_handle, scratch_buffer_1.m_buffer.m_handle, 2, buffer_copies.data()
    );
    ...
    descriptor_set = std::make_shared<vren::pooled_vk_descriptor_set>(m_context->m_toolbox->m_descriptor_pool.acquire(
        m_discretize_point_light_positions_pipeline.m_descriptor_set_layouts.at(0)
    ));
    ...
    vren::vk_utils::write_buffer_descriptor(
        *m_context,
        ...
        2,
        scratch_buffer_1.m_buffer.m_handle,
        point_light_count * sizeof(glm::uvec2),
        0
    );

    m_discretize_point_light_positions_pipeline.bind_descriptor_set(
        command_buffer, 0, descriptor_set->m_handle.m_descriptor_set
    );
    ...
    m_context->m_toolbox->m_bucket_sort(
        command_buffer, resource_container, scratch_buffer_1, point_light_count, 0, scratch_buffer_2, 0
    );
    ...
    buffer_memory_barrier = {
        // Also wait for scratch_buffer_1 to be free as it will be used to store the BVH
        .sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,
        .pNext = nullptr,
    ...
    vren::vk_utils::write_buffer_descriptor(
        *m_context,
        descriptor_set->m_handle.m_descriptor_set,
        0,
        point_light_buffer.m_buffer.m_handle,
        point_light_count * sizeof(vren::point_light),
        0
    );
    vren::vk_utils::write_buffer_descriptor(
        *m_context,
    ...
    vren::vk_utils::write_buffer_descriptor(
        *m_context,
        descriptor_set->m_handle.m_descriptor_set,
        2,
        scratch_buffer_2.m_buffer.m_handle,
        point_light_count * sizeof(glm::uvec2),
        0
    );
    vren::vk_utils::write_buffer_descriptor(
        *m_context,
        ...
        3,
        scratch_buffer_1.m_buffer.m_handle,
        light_count_for_bvh * sizeof(vren::bvh_node),
        0
    );
    ...
    m_context->m_toolbox->m_build_bvh(command_buffer, resource_container, scratch_buffer_1, light_count_for_bvh);
}
...
vren::clustered_shading::find_unique_cluster_list::find_unique_cluster_list(vren::context const& context) :
    ...
    m_pipeline(
        ...
        {
            vren::shader_module shader_module = vren::load_shader_module_from_file(
                context, ".vren/resources/shaders/clustered_shading/find_unique_clusters.comp.spv"
            );
            vren::specialized_shader shader = vren::specialized_shader(shader_module);
            return vren::create_compute_pipeline(context, shader);
        }()
...
)
{
    ...
    struct
    {
        ...
        uint32_t m_z;
        uint32_t _pad;
    } cluster_key_dispatch_params_data;
    ...
    vkCmdUpdateBuffer(
        command_buffer,
        cluster_key_dispatch_params_buffer.m_buffer.m_handle,
        0,
        sizeof(cluster_key_dispatch_params_data),
        &cluster_key_dispatch_params_data
    );
    ...
    VkBufferMemoryBarrier buffer_memory_barrier{
        ...
        .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
        .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
        .buffer = cluster_key_dispatch_params_buffer.m_buffer.m_handle,
        .offset = 0,
    ...
    vren::vk_utils::write_buffer_descriptor(
        *m_context,
        ...
        0,
        cluster_key_buffer.m_buffer.m_handle,
        VK_WHOLE_SIZE,
        0
    );
    vren::vk_utils::write_buffer_descriptor(
    ...
    vren::vk_utils::write_storage_image_descriptor(
        *m_context,
        descriptor_set_1->m_handle.m_descriptor_set,
        2,
        cluster_reference_buffer.m_image_view.m_handle,
    ...
    resource_container.add_resources(descriptor_set_0, descriptor_set_1);
}
...
vren::clustered_shading::assign_lights::assign_lights(vren::context const& context) :
    m_context(&context),
    m_count_pipeline(
        [&]()
        {
            vren::shader_module shader_module = vren::load_shader_module_from_file(
                context, ".vren/resources/shaders/clustered_shading/assign_lights_count.comp.spv"
            );
            vren::specialized_shader shader = vren::specialized_shader(shader_module);
            return vren::create_compute_pipeline(context, shader);
        }()
    ...
    m_write_pipeline(
        ...
        {
            vren::shader_module shader_module = vren::load_shader_module_from_file(
                context, ".vren/resources/shaders/clustered_shading/assign_lights_write.comp.spv"
            );
            vren::specialized_shader shader = vren::specialized_shader(shader_module);
            return vren::create_compute_pipeline(context, shader);
        }()
...
)
{
    ...
    vkCmdFillBuffer(command_buffer, assigned_light_counts_buffer.m_buffer.m_handle, 0, VK_WHOLE_SIZE, 0);
    ...
    if (light_count > 0)
    {
        buffer_memory_barrier = {
            ...
            .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
            .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
            .buffer = assigned_light_counts_buffer.m_buffer.m_handle,
            .offset = 0,
            .size = VK_WHOLE_SIZE};
        vkCmdPipelineBarrier(
            command_buffer,
        ...
        // ------------------------------------------------------------------------------------------------
        // 1. Count the number of light assignments per cluster
        // ------------------------------------------------------------------------------------------------

        vkCmdSetCheckpointNV(command_buffer, "clustered_shading/assign_lights/count");
        ...
        // Push constants
        {
            struct
            {
                ...
                uint32_t m_bvh_root_index;
                uint32_t m_bvh_level_count;
                float _pad1[2];
            } push_constants;

            push_constants = {
                .m_num_tiles = glm::uvec2(vren::divide_and_ceil(screen.x, 32u), vren::divide_and_ceil(screen.y, 32u)),
                .m_camera_near = camera.m_near_plane,
                .m_camera_half_fov_y = camera.m_fov_y / 2.0f,
                .m_camera_proj = camera.get_projection(),
                .m_bvh_root_index = light_bvh_root_index,
                .m_bvh_level_count = vren::calc_bvh_level_count(light_count)};

            m_count_pipeline.push_constants(
                command_buffer, VK_SHADER_STAGE_COMPUTE_BIT, &push_constants, sizeof(push_constants), 0
            );
        }
        ...
        descriptor_set = std::make_shared<vren::pooled_vk_descriptor_set>(
            m_context->m_toolbox->m_descriptor_pool.acquire(m_count_pipeline.m_descriptor_set_layouts.at(0))
        );
        vren::vk_utils::write_buffer_descriptor(
            *m_context,
            descriptor_set->m_handle.m_descriptor_set,
        ...
        vren::vk_utils::write_buffer_descriptor(
            *m_context,
            descriptor_set->m_handle.m_descriptor_set,
            2,
            light_bvh_buffer.m_buffer.m_handle,
            VK_WHOLE_SIZE,
            0
        );
        vren::vk_utils::write_buffer_descriptor(
            *m_context,
            descriptor_set->m_handle.m_descriptor_set,
            3,
            light_index_buffer.m_buffer.m_handle,
            VK_WHOLE_SIZE,
            0
        );
        vren::vk_utils::write_buffer_descriptor(
            *m_context,
            descriptor_set->m_handle.m_descriptor_set,
            4,
            assigned_light_indices_buffer.m_buffer.m_handle,
            VK_WHOLE_SIZE,
            0
        );
        vren::vk_utils::write_buffer_descriptor(
            *m_context,
            descriptor_set->m_handle.m_descriptor_set,
            5,
            assigned_light_counts_buffer.m_buffer.m_handle,
            VK_WHOLE_SIZE,
            0
        );
        vren::vk_utils::write_buffer_descriptor(
            *m_context,
            descriptor_set->m_handle.m_descriptor_set,
            6,
            assigned_light_offsets_buffer.m_buffer.m_handle,
            VK_WHOLE_SIZE,
            0
        );
        vren::vk_utils::write_buffer_descriptor(
        ...
        // Dispatch
        vkCmdDispatchIndirect(command_buffer, cluster_key_dispatch_params_buffer.m_buffer.m_handle, 0);

        resource_container.add_resources(descriptor_set);

        // ------------------------------------------------------------------------------------------------
        // 2. Copy counts buffer to offsets buffer in order to exclusive-scan
        // ------------------------------------------------------------------------------------------------

        vkCmdSetCheckpointNV(command_buffer, "clustered_shading/assign_lights/copy_count_buffer_to_offset_buffer");
        ...
        VkBufferCopy buffer_copy{
            .srcOffset = 0, .dstOffset = 0, .size = VREN_MAX_UNIQUE_CLUSTER_KEY_COUNT * sizeof(uint32_t)};
        vkCmdCopyBuffer(
            command_buffer,
            assigned_light_counts_buffer.m_buffer.m_handle,
            assigned_light_offsets_buffer.m_buffer.m_handle,
        ...
        // ------------------------------------------------------------------------------------------------
        // 3. Use exclusive-scan to find indices offsets
        // ------------------------------------------------------------------------------------------------

        vkCmdSetCheckpointNV(command_buffer, "clustered_shading/assign_lights/exclusive_scan");
        ...
        buffer_memory_barrier = {
            ...
            .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
            .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
            .buffer = assigned_light_offsets_buffer.m_buffer.m_handle,
            .offset = 0,
            .size = VK_WHOLE_SIZE};
        vkCmdPipelineBarrier(
            command_buffer,
        ...
        m_context->m_toolbox->m_blelloch_scan( // TODO : use indirect dispatch ?
            command_buffer,
            resource_container,
            assigned_light_offsets_buffer,
            VREN_MAX_UNIQUE_CLUSTER_KEY_COUNT, // The unique clsuter keys actual count is not known by the host
            0,
            1
        );

        // ------------------------------------------------------------------------------------------------
        // 4. Finally write point light indices
        // ------------------------------------------------------------------------------------------------

        vkCmdSetCheckpointNV(command_buffer, "clustered_shading/assign_lights/write");
        ...
        // Push constants
        {
            ...
            m_write_pipeline.push_constants(
                command_buffer, VK_SHADER_STAGE_COMPUTE_BIT, &push_constants, sizeof(push_constants), 0
            );
        }
        ...
        // Bind descriptor set 0
        descriptor_set = std::make_shared<vren::pooled_vk_descriptor_set>(
            m_context->m_toolbox->m_descriptor_pool.acquire(m_write_pipeline.m_descriptor_set_layouts.at(0))
        );
        vren::vk_utils::write_buffer_descriptor(
            *m_context,
            descriptor_set->m_handle.m_descriptor_set,
...
vren::clustered_shading::shade::shade(vren::context const& context) :
    m_context(&context),
    m_pipeline(
        [&]()
        {
            vren::shader_module shader_module =
                vren::load_shader_module_from_file(context, ".vren/resources/shaders/clustered_shading/shade.comp.spv");
            vren::specialized_shader shader = vren::specialized_shader(shader_module);
            return vren::create_compute_pipeline(context, shader);
        }()
    )
...
)
{
    ...
    // Descriptors
    {
        ...
        vren::vk_utils::write_storage_image_descriptor(
            *m_context, descriptor_set_5->m_handle.m_descriptor_set, 0, output.get_image_view(), VK_IMAGE_LAYOUT_GENERAL
        );
        ...
        resource_container.add_resources(
            descriptor_set_0, descriptor_set_1, descriptor_set_2, descriptor_set_3, descriptor_set_4, descriptor_set_5
        );
    }
...
vren::cluster_and_shade::cluster_and_shade(vren::context const& context) :
    m_context(&context),
    ...
    m_view_space_point_light_position_buffer(
        [&]()
        {
            auto buffer = vren::vk_utils::alloc_device_only_buffer(
                *m_context, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VREN_MAX_POINT_LIGHT_COUNT * sizeof(glm::vec4)
            );
        ...
        [&]()
        {
            auto buffer = vren::vk_utils::alloc_device_only_buffer(
                *m_context,
                ...
                vren::clustered_shading::construct_point_light_bvh::get_required_bvh_buffer_size(
    ...
    m_point_light_index_buffer(
        [&]()
        {
            auto buffer = vren::vk_utils::alloc_device_only_buffer(
                *m_context,
                vren::clustered_shading::construct_point_light_bvh::get_required_point_light_index_buffer_usage_flags(),
                vren::clustered_shading::construct_point_light_bvh::get_required_point_light_index_buffer_size(
                    VREN_MAX_POINT_LIGHT_COUNT
                )
            );
    ...
    m_cluster_key_buffer(vren::vk_utils::alloc_device_only_buffer(
        *m_context, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VREN_MAX_UNIQUE_CLUSTER_KEY_COUNT * sizeof(uint32_t)
    )),
    m_cluster_key_dispatch_params_buffer(vren::vk_utils::alloc_device_only_buffer(
        *m_context,
    ...
    m_cluster_reference_buffer(
        [&]()
        {
            ...
            vren::vk_image_view image_view = vren::vk_utils::create_image_view(
                *m_context, image.m_image.m_handle, VK_FORMAT_R32_UINT, VK_IMAGE_ASPECT_COLOR_BIT
            );

            return vren::vk_utils::combined_image_view{
                .m_image = std::move(image), .m_image_view = std::move(image_view)};
        }()
    ),
    m_assigned_light_indices_buffer(vren::vk_utils::alloc_device_only_buffer(
        *m_context, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VREN_MAX_ASSIGNED_LIGHT_COUNT * sizeof(uint32_t)
    )),
    m_assigned_light_counts_buffer(vren::vk_utils::alloc_device_only_buffer(
        *m_context,
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        VREN_MAX_UNIQUE_CLUSTER_KEY_COUNT * sizeof(uint32_t)
    )),
    m_assigned_light_offsets_buffer(vren::vk_utils::alloc_device_only_buffer(
        *m_context,
    ...
    ))
{
    ...
    vren::vk_utils::set_name(*m_context, m_cluster_key_dispatch_params_buffer, "cluster_key_dispatch_params_buffer");
    vren::vk_utils::set_name(*m_context, m_cluster_reference_buffer, "cluster_reference_buffer");

    vren::vk_utils::set_name(*m_context, m_assigned_light_indices_buffer, "assigned_light_indices_buffer");
    vren::vk_utils::set_name(*m_context, m_assigned_light_counts_buffer, "assigned_light_counts_buffer");
    vren::vk_utils::set_name(*m_context, m_assigned_light_offsets_buffer, "assigned_light_offsets_buffer");
    ...
    node->set_callback(
        ...
        )
        {
            ...
            m_find_unique_cluster_list(
                ...
                m_cluster_key_dispatch_params_buffer,
                m_cluster_reference_buffer
            );
            ...
            buffer_memory_barriers = {
                ...
                VkBufferMemoryBarrier{
                    ...
                    .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
                    .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
                    .buffer = m_cluster_key_dispatch_params_buffer.m_buffer.m_handle,
                    .offset = 0,
                    .size = VK_WHOLE_SIZE},
            };
            ...
            m_assign_lights(
                ...
                m_point_light_index_buffer,
                m_assigned_light_indices_buffer,
                m_assigned_light_counts_buffer,
                m_assigned_light_offsets_buffer,
                m_view_space_point_light_position_buffer
            );
            ...
            buffer_memory_barriers = {
                VkBufferMemoryBarrier{
                    ...
                    .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
                    .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
                    .buffer = m_assigned_light_counts_buffer.m_buffer.m_handle,
                    .offset = 0,
                    .size = VK_WHOLE_SIZE},
                VkBufferMemoryBarrier{
                    .sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,
                    ...
                    .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
                    .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
                    .buffer = m_assigned_light_indices_buffer.m_buffer.m_handle,
                    .offset = 0,
                    .size = VK_WHOLE_SIZE},
            };
            vkCmdPipelineBarrier(
